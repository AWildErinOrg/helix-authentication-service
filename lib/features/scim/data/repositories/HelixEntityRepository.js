...
class HelixEntityRepository extends EntityRepository {
  constructor({ getProvisioningServers, settingsRepository }) {
    super()
    this.autoGenPassword = settingsRepository?.getBool('USER_AUTO_GEN_PASSWD')
  ...
  async addUser(user, server, domain) {
    ...
    } else if (this.autoGenPassword) {
      model.password = `Aa1!${crypto.randomBytes(32).toString('base64')}`
    }
